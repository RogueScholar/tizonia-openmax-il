...
/**
 * @file   tiztuneinmgr.cpp
 * @author Juan A. Rubio <juan.rubio@aratelia.com>
 *
 * @brief  Manager implementation for the Tunein client graph
 *
 */
...
#include "tiztuneinmgr.hpp"

#ifdef TIZ_LOG_CATEGORY_NAME
#undef TIZ_LOG_CATEGORY_NAME
#define TIZ_LOG_CATEGORY_NAME "tiz.play.tuneinmgr"
#endif
...
// mgr
//
graphmgr::tuneinmgr::tuneinmgr (tizgraphconfig_ptr_t config)
  : graphmgr::mgr (), config_ (config)
{
  TIZ_LOG (TIZ_PRIORITY_TRACE, "Constructing...");
}

graphmgr::tuneinmgr::~tuneinmgr ()
{
}

graphmgr::ops *graphmgr::tuneinmgr::do_init (
    ...
    graphmgr_capabilities_t &graphmgr_caps)
{
  ...
  return new tuneinmgrops (this, playlist, termination_cback);
}

//
// decodemgrops
//
graphmgr::tuneinmgrops::tuneinmgrops (
...
tizgraph_ptr_t graphmgr::tuneinmgrops::get_graph (const std::string & /* uri */)
{
  ...
  {
    g_ptr = boost::make_shared< tiz::graph::tunein > ();
    if (g_ptr)
    {
      // TODO: Check rc
    ...
    {
      GMGR_OPS_RECORD_ERROR (OMX_ErrorInsufficientResources,
                             "Unable to create the Tunein client graph.");
    }
  }
  else
  {
...
void graphmgr::tuneinmgrops::do_load ()
{
...
void graphmgr::tuneinmgrops::do_execute ()
{
  assert (playlist_);
  assert (p_mgr_);

  tuneinmgr *p_clientmgr = dynamic_cast< tuneinmgr * > (p_mgr_);
  assert (p_clientmgr);
...
bool graphmgr::tuneinmgrops::is_fatal_error (const OMX_ERRORTYPE error,

...
/**
 * @file   tizyoutubemgr.cpp
 * @author Juan A. Rubio <juan.rubio@aratelia.com>
 *
 * @brief  Manager implementation for the Youtube client graph
 *
 */
...
#include "tizyoutubemgr.hpp"

#ifdef TIZ_LOG_CATEGORY_NAME
#undef TIZ_LOG_CATEGORY_NAME
#define TIZ_LOG_CATEGORY_NAME "tiz.play.youtubemgr"
#endif
...
// mgr
//
graphmgr::youtubemgr::youtubemgr (tizgraphconfig_ptr_t config)
  : graphmgr::mgr (), config_ (config)
{
  TIZ_LOG (TIZ_PRIORITY_TRACE, "Constructing...");
}

graphmgr::youtubemgr::~youtubemgr ()
{
}

graphmgr::ops *graphmgr::youtubemgr::do_init (
    ...
    graphmgr_capabilities_t &graphmgr_caps)
{
  ...
  return new youtubemgrops (this, playlist, termination_cback);
}

//
// decodemgrops
//
graphmgr::youtubemgrops::youtubemgrops (
...
tizgraph_ptr_t graphmgr::youtubemgrops::get_graph (
    const std::string & /* uri */)
{
  ...
  if (it == graph_registry_.end ())
  {
    ...
    {
      GMGR_OPS_RECORD_ERROR (OMX_ErrorInsufficientResources,
                             "Unable to create the Youtube client graph.");
    }
  }
  else
  {
...
void graphmgr::youtubemgrops::do_load ()
{
...
void graphmgr::youtubemgrops::do_execute ()
{
  assert (playlist_);
  assert (p_mgr_);
...
bool graphmgr::youtubemgrops::is_fatal_error (const OMX_ERRORTYPE error,

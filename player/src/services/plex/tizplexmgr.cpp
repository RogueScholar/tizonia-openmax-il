 ...
 *
 * @brief  Manager implementation for the Plex client graph
 *
 */
...
#include "tizplexmgr.hpp"

#ifdef TIZ_LOG_CATEGORY_NAME
#undef TIZ_LOG_CATEGORY_NAME
#define TIZ_LOG_CATEGORY_NAME "tiz.play.plexmgr"
#endif
...
// mgr
//
graphmgr::plexmgr::plexmgr (tizgraphconfig_ptr_t config)
  : graphmgr::mgr (), config_ (config)
{
  TIZ_LOG (TIZ_PRIORITY_TRACE, "Constructing...");
}

graphmgr::plexmgr::~plexmgr ()
{
}

graphmgr::ops *graphmgr::plexmgr::do_init (
    ...
    graphmgr_capabilities_t &graphmgr_caps)
{
  ...
  return new plexmgrops (this, playlist, termination_cback);
}

//
// decodemgrops
//
graphmgr::plexmgrops::plexmgrops (
...
tizgraph_ptr_t graphmgr::plexmgrops::get_graph (const std::string & /* uri */)
{
  ...
  if (it == graph_registry_.end ())
  {
    ...
    {
      GMGR_OPS_RECORD_ERROR (OMX_ErrorInsufficientResources,
                             "Unable to create the Plex client graph.");
    }
  }
  else
  {
...
void graphmgr::plexmgrops::do_load ()
{
...
void graphmgr::plexmgrops::do_execute ()
{
  assert (playlist_);
  assert (p_mgr_);

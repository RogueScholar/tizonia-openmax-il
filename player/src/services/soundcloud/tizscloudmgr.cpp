...
/**
 * @file   tizscloudmgr.cpp
 * @author Juan A. Rubio <juan.rubio@aratelia.com>
 *
 * @brief  Manager implementation for the SoundCloud client graph
 *
 */
...
#include "tizscloudgraph.hpp"
#include "tizscloudmgr.hpp"

#ifdef TIZ_LOG_CATEGORY_NAME
#undef TIZ_LOG_CATEGORY_NAME
#define TIZ_LOG_CATEGORY_NAME "tiz.play.scloudmgr"
#endif
...
// mgr
//
graphmgr::scloudmgr::scloudmgr (tizgraphconfig_ptr_t config)
  : graphmgr::mgr (), config_ (config)
{
  TIZ_LOG (TIZ_PRIORITY_TRACE, "Constructing...");
}

graphmgr::scloudmgr::~scloudmgr ()
{
}

graphmgr::ops *graphmgr::scloudmgr::do_init (
    ...
    graphmgr_capabilities_t &graphmgr_caps)
{
  ...
  return new scloudmgrops (this, playlist, termination_cback);
}

//
// decodemgrops
//
graphmgr::scloudmgrops::scloudmgrops (
...
tizgraph_ptr_t graphmgr::scloudmgrops::get_graph (const std::string & /* uri */)
{
  ...
  {
    g_ptr = boost::make_shared< tiz::graph::scloud > ();
    if (g_ptr)
    {
      // TODO: Check rc
    ...
    {
      GMGR_OPS_RECORD_ERROR (OMX_ErrorInsufficientResources,
                             "Unable to create the SoundCloud client graph.");
    }
  }
  else
  {
...
void graphmgr::scloudmgrops::do_load ()
{
...
void graphmgr::scloudmgrops::do_execute ()
{
  assert (playlist_);
  assert (p_mgr_);

  scloudmgr *p_clientmgr = dynamic_cast< scloudmgr * > (p_mgr_);
  assert (p_clientmgr);
...
bool graphmgr::scloudmgrops::is_fatal_error (const OMX_ERRORTYPE error,

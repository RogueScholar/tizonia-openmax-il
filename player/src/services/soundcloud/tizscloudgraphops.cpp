...
/**
 * @file   tizscloudgraphops.cpp
 * @author Juan A. Rubio <juan.rubio@aratelia.com>
 *
 * @brief  SoundCloud client graph actions / operations implementation
 *
 */
...
#define TIZ_LOG_CATEGORY_NAME "tiz.play.graph.scloud.ops"
#endif
...
// scloudops
//
graph::scloudops::scloudops (graph *p_graph,
...
void graph::scloudops::do_load ()
{
  ...
  // graph, the audio decoder and the pcm renderer. The scloud source is already
  // instantiated and in
  // Executing state.
  ...
  G_OPS_BAIL_IF_ERROR (
      get_encoding_type_from_scloud_source (),
      "Unable to retrieve the audio encoding from the scloud source.");
...
void graph::scloudops::do_reconfigure_tunnel (const int tunnel_id)
{
  if (last_op_succeeded ())
  {
    if (0 == tunnel_id)
                                 ...
                                 const OMX_COMMANDTYPE to_disabled_or_enabled)
{
  ...
  {
    const int scloud_source_index = 0;
    const int scloud_source_output_port = 0;
    add_expected_port_transition (handles_[scloud_source_index],
                                  scloud_source_output_port,
...
OMX_ERRORTYPE
graph::scloudops::override_decoder_and_renderer_sampling_rates ()
{
  OMX_U32 channels = 2;
...
OMX_ERRORTYPE
graph::scloudops::apply_pcm_codec_info_from_decoder ()
{
  OMX_U32 channels = 2;
  OMX_U32 sampling_rate = 44100;
  std::string encoding_str;
...
OMX_ERRORTYPE
graph::scloudops::get_channels_and_rate_from_decoder (
...
OMX_ERRORTYPE
graph::scloudops::set_channels_and_rate_on_decoder (const OMX_U32 channels,
...
void graph::scloudops::do_reconfigure_first_tunnel ()
{
  // Retrieve the mp3 settings from the scloud source component
  OMX_AUDIO_PARAM_MP3TYPE scloud_mp3type;
  const OMX_U32 scloud_port_id = 0;
  TIZ_INIT_OMX_PORT_STRUCT (scloud_mp3type, scloud_port_id);
  G_OPS_BAIL_IF_ERROR (
      OMX_GetParameter (handles_[0], OMX_IndexParamAudioMp3, &scloud_mp3type),
      "Unable to retrieve the MP3 settings from the scloud source");
  ...
  decoder_mp3type.nChannels = scloud_mp3type.nChannels;
  decoder_mp3type.nSampleRate = scloud_mp3type.nSampleRate;
...
void graph::scloudops::do_reconfigure_second_tunnel ()
{

...
{
  G_OPS_BAIL_IF_ERROR (
      probe_stream (OMX_PortDomainAudio, OMX_AUDIO_CodingOPUS, "OggOpus",
...
void graph::oggopusdecops::do_configure ()
{
  ...
  G_OPS_BAIL_IF_ERROR (
      tiz::graph::util::set_pcm_mode (
          handles_[2], 0,
          boost::bind (&tiz::graph::oggopusdecops::get_pcm_codec_info, this,
                       _1)),
      "Unable to set OMX_IndexParamAudioPcm");
}

void graph::oggopusdecops::get_pcm_codec_info (
...
bool graph::oggopusdecops::probe_stream_hook ()
{
  if (probe_ptr_)
  {
    OMX_AUDIO_PARAM_PCMMODETYPE pcmtype;

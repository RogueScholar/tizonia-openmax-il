...
namespace tiz
{
  namespace graphmgr
  {
    static char const* const state_names[]
        = { "inited",   "starting", "running",  "restarting",
            "stopping", "stopped",  "quitting", "quitted" };
    ...
    struct vol_evt
    {
      vol_evt (const double vol) : vol_ (vol)
      {
      }
      const double vol_;
    };
    ...
    {
      graph_volume_evt (const int& volume) : volume_ (volume)
      {
      }
      const int volume_;
    };
    ...
    struct fsm_ : public boost::msm::front::state_machine_def< fsm_ >
    {
      ...
      struct starting_
        : public boost::msm::front::state_machine_def< starting_ >
      {
        // no need for exception handling
        typedef int no_exception_thrown;
        ...
        starting_ () : pp_ops_ (NULL)
        {
        }
        starting_ (ops** pp_ops) : pp_ops_ (pp_ops)
        {
          assert (pp_ops);
        }
        ...
        {
          typedef boost::mpl::vector< next_evt, prev_evt, fwd_evt, rwd_evt,
                                      vol_up_evt, vol_down_evt, vol_evt,
                                      mute_evt, pause_evt, stop_evt, quit_evt >
              deferred_events;
          template < class Event, class FSM >
        ...
        struct starting_exit
          : public boost::msm::front::exit_pseudo_state< graph_execd_evt >
        {
          typedef boost::mpl::vector< next_evt, prev_evt, fwd_evt, rwd_evt,
                                      vol_up_evt, vol_down_evt, vol_evt,
                                      mute_evt, pause_evt, stop_evt, quit_evt >
        ...
        struct transition_table
          : boost::mpl::vector<
                ...
                //       Guard
                //    +--+-------------------+-------------------+-------------------+------------------+-----+
                bmf::Row< loading_graph, graph_loaded_evt, executing_graph,
                          do_execute_graph >,
      ...
      // typedef boost::msm::back::state_machine<starting_,
      // boost::msm::back::mpl_graph_fsm_check> starting;
      typedef boost::msm::back::state_machine< starting_ > starting;

      /* restarting is a submachine */
      struct restarting_
        : public boost::msm::front::state_machine_def< restarting_ >
      {
        // no need for exception handling
        typedef int no_exception_thrown;
        ...
        restarting_ () : pp_ops_ (NULL)
        {
        }

        restarting_ (ops** pp_ops) : pp_ops_ (pp_ops)
        {
          assert (pp_ops);
        }

        struct restarting_exit
          : public boost::msm::front::exit_pseudo_state< graph_unlded_evt >
        {
          typedef boost::mpl::vector< next_evt, prev_evt, fwd_evt, rwd_evt,
                                      vol_up_evt, vol_down_evt, vol_evt,
                                      mute_evt, pause_evt, stop_evt, quit_evt >
      ...
      // typedef boost::msm::back::state_machine<restarting_,
      // boost::msm::back::mpl_graph_fsm_check> restarting;
      typedef boost::msm::back::state_machine< restarting_ > restarting;

      /* stopping is a submachine */
      struct stopping_
        : public boost::msm::front::state_machine_def< stopping_ >
      {
        // no need for exception handling
        typedef int no_exception_thrown;
        ...
        stopping_ () : pp_ops_ (NULL)
        {
        }

        stopping_ (ops** pp_ops) : pp_ops_ (pp_ops)
        {
          assert (pp_ops);
        }

        // The list of FSM states
        struct stopping_exit
          : public boost::msm::front::exit_pseudo_state< graph_stopped_evt >
        {
          template < class Event, class FSM >
          void on_entry (Event const&, FSM&)
          {
      ...
      // typedef boost::msm::back::state_machine<stopping_,
      // boost::msm::back::mpl_graph_fsm_check> stopping;
      typedef boost::msm::back::state_machine< stopping_ > stopping;

      /* quitting is a submachine */
      struct quitting_
        : public boost::msm::front::state_machine_def< quitting_ >
      {
        // no need for exception handling
        typedef int no_exception_thrown;
        ...
        quitting_ () : pp_ops_ (NULL)
        {
        }

        quitting_ (ops** pp_ops) : pp_ops_ (pp_ops)
        {
          assert (pp_ops);
        }

        // The list of FSM states
        struct quitting_exit
          : public boost::msm::front::exit_pseudo_state< graph_unlded_evt >
        {
          template < class Event, class FSM >
          void on_entry (Event const&, FSM&)
      ...
      // typedef boost::msm::back::state_machine<quitting_,
      // boost::msm::back::mpl_graph_fsm_check> quitting;
      typedef boost::msm::back::state_machine< quitting_ > quitting;

      struct executing_graph : public boost::msm::front::state<>
      {
        typedef boost::mpl::vector< next_evt, prev_evt, fwd_evt, rwd_evt,
                                    vol_up_evt, vol_down_evt, vol_evt, mute_evt,
                                    pause_evt, stop_evt, quit_evt >
            deferred_events;
        template < class Event, class FSM >
        void on_entry (Event const&, FSM&)
        {
      ...
      struct is_fatal_error
      {
        ...
        bool operator() (EVT const& evt, FSM& fsm, SourceState&, TargetState&)
        {
          ...
          else
          {
            ...
            {
              rc = (*(fsm.pp_ops_))
                       ->is_fatal_error (evt.error_code_, evt.error_str_);
            }
          }

          TIZ_LOG (TIZ_PRIORITY_ERROR, "is_fatal_error [%s]",
                   rc ? "YES" : "NO");
          return rc;
        }
      };
      ...
      struct transition_table
        : boost::mpl::vector<
              ...
              //    +----+---------------------+------------------+-------------+------------------------+--------------------+
              bmf::Row< starting ::exit_pt< starting_ ::starting_exit >,
                        graph_execd_evt, running >,
              bmf::Row< starting, err_evt, restarting, bmf::none,
                        bmf::euml::Not_< is_fatal_error > >,
              bmf::Row< starting, quit_evt, quitting, do_unload >,
              bmf::Row< starting, err_evt, quitted, do_report_fatal_error,
                        is_fatal_error >,
              ...
              bmf::Row< running, graph_paused_evt, bmf::none,
                        do_update_control_ifcs< tc::Paused > >,
              bmf::Row< running, graph_resumed_evt, bmf::none,
                        do_update_control_ifcs< tc::Playing > >,
              bmf::Row< running, graph_metadata_evt, bmf::none,
                        do_update_metadata >,
              bmf::Row< running, graph_volume_evt, bmf::none,
                        do_update_volume >,
              ...
              bmf::Row< running, err_evt, restarting, bmf::none,
                        bmf::euml::Not_< is_fatal_error > >,
              bmf::Row< running, err_evt, quitted, do_report_fatal_error,
                        is_fatal_error >,
              //    +----+---------------------+------------------+-------------+------------------------+--------------------+
              bmf::Row< restarting ::exit_pt< restarting_ ::restarting_exit >,
                        graph_unlded_evt, starting,
                        bmf::ActionSequence_<
                            boost::mpl::vector< do_deinit, do_load > > >,
              bmf::Row< restarting, err_evt, quitted, do_report_fatal_error >,
              //    +----+---------------------+------------------+-------------+------------------------+--------------------+
              bmf::Row< stopping ::exit_pt< stopping_ ::stopping_exit >,
                        graph_stopped_evt, stopped >,
              ...
              //    +----+---------------------+------------------+-------------+------------------------+--------------------+
              bmf::Row< quitting ::exit_pt< quitting_ ::quitting_exit >,
                        graph_unlded_evt, quitted >,

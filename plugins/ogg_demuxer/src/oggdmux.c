...
instantiate_filter_ogg_input_port (OMX_HANDLETYPE ap_hdl)
{
  ...
  return factory_new (tiz_get_type (ap_hdl, "tizaudioport"), &port_opts,
                      &encodings);
}
...
instantiate_source_audio_output_port (OMX_HANDLETYPE ap_hdl)
{
  return instantiate_audio_output_port (
    ap_hdl, ARATELIA_OGG_DEMUXER_AUDIO_PORT_BASE_INDEX);
}
...
{
  return instantiate_audio_output_port (
    ap_hdl, ARATELIA_OGG_DEMUXER_AUDIO_PORT_BASE_INDEX + 1);
}
...
instantiate_source_video_output_port (OMX_HANDLETYPE ap_hdl)
{
  return instantiate_video_output_port (
    ap_hdl, ARATELIA_OGG_DEMUXER_VIDEO_PORT_BASE_INDEX);
}
...
{
  return instantiate_video_output_port (
    ap_hdl, ARATELIA_OGG_DEMUXER_VIDEO_PORT_BASE_INDEX + 1);
}
...
instantiate_config_port (OMX_HANDLETYPE ap_hdl)
{
  return factory_new (tiz_get_type (ap_hdl, "tizdemuxercfgport"),
                      ...
                      ARATELIA_OGG_DEMUXER_COMPONENT_NAME, ogg_demuxer_version);
}
...
OMX_ComponentInit (OMX_HANDLETYPE ap_hdl)
{
  ...
  const tiz_role_factory_t * rf_list[]
    = {&source_demuxer_role, &filter_demuxer_role};
  tiz_type_factory_t oggdmuxprc_type;
  const tiz_type_factory_t * tf_list[] = {&oggdmuxprc_type};

  strcpy ((OMX_STRING) source_demuxer_role.role,
          ARATELIA_OGG_DEMUXER_DEFAULT_ROLE);
  ...
  strcpy ((OMX_STRING) filter_demuxer_role.role,
          ARATELIA_OGG_DEMUXER_FILTER_ROLE);

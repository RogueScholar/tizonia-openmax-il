...
#include "httpsrc.h"
#include "plexprc.h"
#include "plexprc_decls.h"

#ifdef TIZ_LOG_CATEGORY_NAME
#undef TIZ_LOG_CATEGORY_NAME
#define TIZ_LOG_CATEGORY_NAME "tiz.http_source.prc.plex"
#endif

/* forward declarations */
static OMX_ERRORTYPE
plex_prc_deallocate_resources (void *);
static OMX_ERRORTYPE
release_buffer (plex_prc_t *);
static OMX_ERRORTYPE
prepare_for_port_auto_detection (plex_prc_t * ap_prc);
static OMX_ERRORTYPE
plex_prc_prepare_to_transfer (void * ap_prc, OMX_U32 a_pid);
static OMX_ERRORTYPE
...
static OMX_ERRORTYPE
obtain_coding_type (plex_prc_t * ap_prc, char * ap_info)
{
...
static void
obtain_content_length (plex_prc_t * ap_prc, char * ap_info)
{
  char * p_end = NULL;
...
static OMX_ERRORTYPE
set_audio_coding_on_port (plex_prc_t * ap_prc)
{
...
static OMX_ERRORTYPE
set_auto_detect_on_port (plex_prc_t * ap_prc)
{
  OMX_PARAM_PORTDEFINITIONTYPE port_def;
  assert (ap_prc);
...
static OMX_ERRORTYPE
store_metadata (plex_prc_t * ap_prc, const char * ap_header_name,
...
static void
send_port_auto_detect_events (plex_prc_t * ap_prc)
{
  assert (ap_prc);
...
static OMX_ERRORTYPE
update_metadata (plex_prc_t * ap_prc)
{
  assert (ap_prc);
...
static OMX_ERRORTYPE
obtain_next_url (plex_prc_t * ap_prc, int a_skip_value)
{
  OMX_ERRORTYPE rc = OMX_ErrorNone;
...
static OMX_ERRORTYPE
release_buffer (plex_prc_t * ap_prc)
{
  assert (ap_prc);
...
static OMX_ERRORTYPE
retrieve_session_configuration (plex_prc_t * ap_prc)
{
  return tiz_api_GetParameter (
    tiz_get_krn (handleOf (ap_prc)), handleOf (ap_prc),
    OMX_TizoniaIndexParamAudioPlexSession, &(ap_prc->session_));
}

static OMX_ERRORTYPE
retrieve_playlist (plex_prc_t * ap_prc)
{
  return tiz_api_GetParameter (
    tiz_get_krn (handleOf (ap_prc)), handleOf (ap_prc),
    OMX_TizoniaIndexParamAudioPlexPlaylist, &(ap_prc->playlist_));
}
...
static OMX_ERRORTYPE
enqueue_playlist_items (plex_prc_t * ap_prc)
{
  int rc = 1;
...
static void *
plex_prc_ctor (void * ap_obj, va_list * app)
{
  plex_prc_t * p_prc = super_ctor (typeOf (ap_obj, "plexprc"), ap_obj, app);
...
static void *
plex_prc_dtor (void * ap_obj)
{
  (void) plex_prc_deallocate_resources (ap_obj);
  return super_dtor (typeOf (ap_obj, "plexprc"), ap_obj);
}
...
static OMX_ERRORTYPE
plex_prc_allocate_resources (void * ap_obj, OMX_U32 a_pid)
{
  plex_prc_t * p_prc = ap_obj;
...
static OMX_ERRORTYPE
plex_prc_deallocate_resources (void * ap_prc)
{
...
static OMX_ERRORTYPE
plex_prc_stop_and_return (void * ap_prc)
{
  plex_prc_t * p_prc = ap_prc;
  assert (p_prc);
  if (p_prc->p_trans_)
...
static OMX_ERRORTYPE
plex_prc_buffers_ready (const void * ap_prc)
{
  plex_prc_t * p_prc = (plex_prc_t *) ap_prc;
  assert (p_prc);
...
static OMX_ERRORTYPE
plex_prc_pause (const void * ap_obj)
{
  return OMX_ErrorNone;
}

static OMX_ERRORTYPE
plex_prc_resume (const void * ap_obj)
{
  return OMX_ErrorNone;
}

static OMX_ERRORTYPE
plex_prc_port_flush (const void * ap_obj, OMX_U32 TIZ_UNUSED (a_pid))
{
  plex_prc_t * p_prc = (plex_prc_t *) ap_obj;
  if (p_prc->p_trans_)
    {
      tiz_urltrans_flush_buffer (p_prc->p_trans_);
    }
...
static OMX_ERRORTYPE
plex_prc_port_disable (const void * ap_obj, OMX_U32 TIZ_UNUSED (a_pid))
{
  plex_prc_t * p_prc = (plex_prc_t *) ap_obj;
  assert (p_prc);
  ...
  return release_buffer ((plex_prc_t *) ap_obj);
}

static OMX_ERRORTYPE
plex_prc_port_enable (const void * ap_prc, OMX_U32 a_pid)
{
  plex_prc_t * p_prc = (plex_prc_t *) ap_prc;
  OMX_ERRORTYPE rc = OMX_ErrorNone;
  assert (p_prc);
...
static void *
plex_prc_class_ctor (void * ap_obj, va_list * app)
{
  /* NOTE: Class methods might be added in the future. None for now. */
  return super_ctor (typeOf (ap_obj, "plexprc_class"), ap_obj, app);
}
...
plex_prc_class_init (void * ap_tos, void * ap_hdl)
{
  ...
  void * plexprc_class = factory_new
    /* TIZ_CLASS_COMMENT: class type, class name, parent, size */
    (classOf (tizprc), "plexprc_class", classOf (tizprc),
     sizeof (plex_prc_class_t),
     /* TIZ_CLASS_COMMENT: */
     ap_tos, ap_hdl,
     /* TIZ_CLASS_COMMENT: class constructor */
     ctor, plex_prc_class_ctor,
     /* TIZ_CLASS_COMMENT: stop value*/
     0);
  return plexprc_class;
}
...
{
  void * tizprc = tiz_get_type (ap_hdl, "tizprc");
  void * plexprc_class = tiz_get_type (ap_hdl, "plexprc_class");
  TIZ_LOG_CLASS (plexprc_class);
  void * plexprc = factory_new
    /* TIZ_CLASS_COMMENT: class type, class name, parent, size */
    (plexprc_class, "plexprc", tizprc, sizeof (plex_prc_t),
     /* TIZ_CLASS_COMMENT: */
     ap_tos, ap_hdl,
     /* TIZ_CLASS_COMMENT: class constructor */
     ctor, plex_prc_ctor,
     ...
     dtor, plex_prc_dtor,
     /* TIZ_CLASS_COMMENT: */
     tiz_srv_allocate_resources, plex_prc_allocate_resources,
     /* TIZ_CLASS_COMMENT: */
     tiz_srv_deallocate_resources, plex_prc_deallocate_resources,
     /* TIZ_CLASS_COMMENT: */
     tiz_srv_prepare_to_transfer, plex_prc_prepare_to_transfer,
     /* TIZ_CLASS_COMMENT: */
     tiz_srv_transfer_and_process, plex_prc_transfer_and_process,
     /* TIZ_CLASS_COMMENT: */
     tiz_srv_stop_and_return, plex_prc_stop_and_return,
     /* TIZ_CLASS_COMMENT: */
     tiz_srv_io_ready, plex_prc_io_ready,
     /* TIZ_CLASS_COMMENT: */
     tiz_srv_timer_ready, plex_prc_timer_ready,
     /* TIZ_CLASS_COMMENT: */
     tiz_prc_buffers_ready, plex_prc_buffers_ready,
     /* TIZ_CLASS_COMMENT: */
     tiz_prc_pause, plex_prc_pause,
     /* TIZ_CLASS_COMMENT: */
     tiz_prc_resume, plex_prc_resume,
     /* TIZ_CLASS_COMMENT: */
     tiz_prc_port_flush, plex_prc_port_flush,
     /* TIZ_CLASS_COMMENT: */
     tiz_prc_port_disable, plex_prc_port_disable,
     /* TIZ_CLASS_COMMENT: */
     tiz_prc_port_enable, plex_prc_port_enable,
     /* TIZ_CLASS_COMMENT: */
     tiz_prc_config_change, plex_prc_config_change,
     /* TIZ_CLASS_COMMENT: stop value */
     0);

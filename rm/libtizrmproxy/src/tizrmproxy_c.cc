...
get_rm ()
{
  ...
  if (!p_rm)
    {
      p_rm = (tiz_rm_int_t *) tiz_mem_calloc (1, sizeof (tiz_rm_int_t));
      ...
      if (OMX_ErrorNone != (rc = tiz_sem_init (&(p_rm->sem), 0)))
        {
          TIZ_LOG (TIZ_PRIORITY_TRACE, "Error Initializing rm...");
          return NULL;
        }

      if (OMX_ErrorNone != (rc = tiz_queue_init (&(p_rm->p_queue), 10)))
        {
          return NULL;
        }
...
tiz_rm_proxy_init (tiz_rm_t * ap_rm, const OMX_STRING ap_name,
                   ...
                   const tiz_rm_proxy_callbacks_t * ap_cbacks, OMX_PTR ap_data)
{
  ...
  if (ETIZRmStateStarting == p_rm->state || ETIZRmStateStopped == p_rm->state)
    {
      ...
      p_rm->p_proxy = new tizrmproxy (*(p_rm->p_connection), TIZ_RM_DAEMON_PATH,
                                      TIZ_RM_DAEMON_NAME);
      ...
      tiz_thread_create (&(p_rm->thread), 0, 0, il_rmproxy_thread_func, p_rm);
  ...
  p_rm->ref_count++;
  if (NULL
      == (*ap_rm = p_rm->p_proxy->register_client (
            ap_name, *ap_uuid, ap_pri->nGroupID, ap_pri->nGroupPriority,
            ap_cbacks->pf_waitend, ap_cbacks->pf_preempt,
            ap_cbacks->pf_preempt_end, ap_data)))
    {
...
tiz_rm_proxy_destroy (tiz_rm_t * ap_rm)
{
  ...
  TIZ_LOG (TIZ_PRIORITY_TRACE, "IL RM Proxy destroy : ref_count [%d]",
           p_rm->ref_count);
...
extern "C" tiz_rm_error_t
tiz_rm_proxy_cancel_wait (const tiz_rm_t * ap_rm, OMX_U32 a_rid,
                          OMX_U32 a_quantity)
{
  tiz_rm_int_t * p_rm = NULL;
  if (!ap_rm)
...
extern "C" tiz_rm_error_t
tiz_rm_proxy_preemption_conf (const tiz_rm_t * ap_rm, OMX_U32 a_rid,
                              OMX_U32 a_quantity)
{
  tiz_rm_int_t * p_rm = NULL;
  if (!ap_rm)
  ...
  return (tiz_rm_error_t) p_rm->p_proxy->preemption_conf (ap_rm, a_rid,
                                                          a_quantity);
}

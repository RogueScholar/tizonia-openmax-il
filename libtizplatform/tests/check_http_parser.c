...
START_TEST (test_http_parser_request_test)
{
  ...
  fail_if (0 != strcmp ("GET", tiz_http_parser_get_method (p_http_parser)));

  TIZ_LOG (TIZ_PRIORITY_TRACE, "Url = [%s]",
           tiz_http_parser_get_url (p_http_parser));

  fail_if (0 != strcmp ("/metallica", tiz_http_parser_get_url (p_http_parser)));

  fail_if (
    0
    != strcmp ("Audacious/3.2.1 neon/0.29.6",
  ...
  fail_if (0
           != strcmp ("TE, close", tiz_http_parser_get_header (p_http_parser,
                                                               "Connection")));

  fail_if (
    0 != strcmp ("trailers", tiz_http_parser_get_header (p_http_parser, "TE")));
  ...
  fail_if (0
           != strcmp (
             "1", tiz_http_parser_get_header (p_http_parser, "Icy-MetaData")));

...
{
  TIZ_LOG (TIZ_PRIORITY_TRACE, "key [%d] value [%d]", *((int *) ap_key),
           *((int *) ap_value));
  tiz_mem_free (ap_key);
}

static OMX_S32
check_map_for_each_f (OMX_PTR ap_key, OMX_PTR ap_value, OMX_PTR ap_arg)
{
  static int g_each = 0;
  ...
  TIZ_LOG (TIZ_PRIORITY_TRACE, "each [%d] key [%d] value [%d]", g_each, *p_key,
           *p_value);
...
START_TEST (test_map_insert_find_erase_size_empty_at_for_each)
{
  ...
  fail_if (OMX_ErrorNone
           != tiz_map_for_each (p_map, check_map_for_each_f, NULL));

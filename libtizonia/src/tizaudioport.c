...
{
    tiz_audioport_t * p_obj
        = super_ctor (typeOf (ap_obj, "tizaudioport"), ap_obj, app);
    ...
    p_obj->port_format_.nSize = (OMX_U32) sizeof (OMX_AUDIO_PARAM_PORTFORMATTYPE);
    ...
    p_obj->port_format_.eEncoding
        = p_encodings ? p_encodings[0] : OMX_AUDIO_CodingUnused;
...
audioport_GetParameter (const void * ap_obj, OMX_HANDLETYPE ap_hdl,
                        OMX_INDEXTYPE a_index, OMX_PTR ap_struct)
{
    ...
    case OMX_IndexParamAudioPortFormat:
    {
        ...
        if ((OMX_S32) p_pft->nIndex
                >= tiz_vector_length (p_obj->p_encodings_))
        {
            return OMX_ErrorNoMore;
        }

        p_encoding
            = tiz_vector_at (p_obj->p_encodings_, (OMX_S32) p_pft->nIndex);
        assert (p_encoding);
    ...
    default:
    {
        ...
        return super_GetParameter (typeOf (ap_obj, "tizaudioport"), ap_obj,
...
static OMX_ERRORTYPE
audioport_SetParameter (const void * ap_obj, OMX_HANDLETYPE ap_hdl,
                        OMX_INDEXTYPE a_index, OMX_PTR ap_struct)
{
    ...
    {
        /* Try the parent's indexes */
        rc = super_SetParameter (typeOf (ap_obj, "tizaudioport"), ap_obj,
                                 ap_hdl, a_index, ap_struct);
    }
    break;
    };
...
tiz_audioport_class_init (void * ap_tos, void * ap_hdl)
{
    ...
    void * tizaudioport_class = factory_new
                                ...
                                (classOf (tizport), "tizaudioport_class", classOf (tizport),

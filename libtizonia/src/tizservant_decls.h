...
#include "tizapi_decls.h"
  ...
  struct tiz_srv_class
  {
    ...
    OMX_ERRORTYPE (*enqueue)
    (const void * ap_obj, OMX_PTR ap_data, OMX_U32 a_priority);
    ...
    OMX_ERRORTYPE (*timer_watcher_init)
    (void * ap_obj, tiz_event_timer_t ** app_ev_timer);
    OMX_ERRORTYPE (*timer_watcher_start)
    (void * ap_obj, tiz_event_timer_t * ap_ev_timer, const double a_after,
     const double a_repeat);
    OMX_ERRORTYPE (*timer_watcher_restart)
    (void * ap_obj, tiz_event_timer_t * ap_ev_timer);
    OMX_ERRORTYPE (*timer_watcher_stop)
    (void * ap_obj, tiz_event_timer_t * ap_ev_timer);
    void (*timer_watcher_destroy) (void * ap_obj,
                                   tiz_event_timer_t * ap_ev_timer);

    OMX_ERRORTYPE (*event_io)
    (void * ap_obj, tiz_event_io_t * ap_ev_io, const uint32_t a_id, int a_fd,
     int a_events);
    OMX_ERRORTYPE (*event_timer)
    (void * ap_obj, tiz_event_timer_t * ap_ev_timer, const uint32_t a_id);
    OMX_ERRORTYPE (*event_stat)
    (void * ap_obj, tiz_event_stat_t * ap_ev_stat, const uint32_t a_id,
     int a_events);

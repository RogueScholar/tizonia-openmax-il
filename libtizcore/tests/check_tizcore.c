...
START_TEST (test_ilcore_init_and_deinit_get_hdl_free_hdl)
{
  ...
  error = OMX_GetHandle (&p_hdl, TIZ_CORE_TEST_COMPONENT_NAME,
                         (OMX_PTR *) (&appData), &callBacks);
  TIZ_LOG (TIZ_PRIORITY_TRACE, "OMX_GetHandle error [%s]",
...
tizcore_suite (void)
{
  ...
  tcase_add_test (tc_ilcore, test_ilcore_init_and_deinit_get_hdl_free_hdl);

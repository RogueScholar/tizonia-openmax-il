...
class tizchromecastproxy(object):
    ...
    def _do_activate(self):
        self.cast = pychromecast.Chromecast(self.ip_addr,
                                            timeout=0.5,
                                            blocking=False)
    ...
    def activate(self, cast_status_listener, media_status_listener):
        print_nfo("[Chromecast] [{0}] [Activating]".format(
    ...
    def poll_socket(self, polltime_ms):
        print_nfo("[Chromecast] [{0}] [poll_socket start]".format(
            to_ascii(self.ip_addr)))
        ...
        if sock and sock.fileno() != -1:
            ...
            # can_read = True
            if can_read:
                print_nfo("[Chromecast] [{0}] [poll_socket can_read]".format(
                    to_ascii(self.ip_addr)))
                ...
                try:
                    ...
                    pass
        print_wrn("[Chromecast] [{0}] [poll_socket end]".format(
            to_ascii(self.ip_addr)))
    ...
    def media_load(
        ...
        print_nfo("[Chromecast] [{0}] [media_load start]".format(
            to_ascii(self.ip_addr)))
        ...
        try:
            ...
            if st.player_is_playing:
                self.cast.media_controller.stop()
                time.sleep(1)
            mc.play_media(url, content_type, title, thumb, current_time,
                          autoplay, stream_type)
        except Exception as exception:
            print_err("Unable to load stream")
            exc_type, value, traceback = sys.exc_info()
            print_exception(exc_type, value, traceback)

        print_wrn("[Chromecast] [{0}] [media_load end]".format(
            to_ascii(self.ip_addr)))
    ...
    def new_cast_status(self, status):
        ...
        print_nfo("[Chromecast] [{0}] [new_cast_status start]".format(
            to_ascii(self.ip_addr)))
        if status:
            try:
                if not status.app_id:
                    ...
                    print_nfo("[Chromecast] [READY_TO_CAST] [new_cast_status]".
                              format(to_ascii(self.ip_addr)))
                ...
                logging.info("Unable to deliver cast status callback %s",
                             exception)
        print_wrn("[Chromecast] [{0}] [new_cast_status end]".format(
            to_ascii(self.ip_addr)))
    ...
    def new_media_status(self, status):
        ...
        print_nfo("[Chromecast] [{0}] [new_media_status start]".format(
            to_ascii(self.ip_addr)))
        if not self.active:
        ...
        if status:
            try:
                print_nfo("[Chromecast] [{0}] [new_media_status : {1}]".format(
                ...
                logging.info("Unable to deliver media status callback %s",
                             exception)
        print_wrn("[Chromecast] [{0}] [new_media_status end]".format(
            to_ascii(self.ip_addr)))

  ...
  namespace cast
  {
    static char const* const state_names[]
        = { "starting", "started",  "connecting", "connected",
            "running",  "quitting", "quitted",    "polling" };
    ...
    struct connect_evt
    {
      connect_evt (const std::string& name_or_ip) : name_or_ip_ (name_or_ip)
      {
      }
    ...
    struct load_url_evt
    {
      load_url_evt (const std::string& url, const std::string& mime_type,
    ...
    struct volume_evt
    {
      volume_evt (int volume) : volume_ (volume)
      {
      }
      const int volume_;
    };
    ...
    struct poll_evt
    {
      poll_evt (int poll_time_ms) : poll_time_ms_ (poll_time_ms)
      {
      }
    ...
    struct fsm_ : public boost::msm::front::state_machine_def< fsm_ >
    {
      ...
      {
        typedef boost::mpl::vector< cast_status_evt, load_url_evt, volume_evt >
            deferred_events;
        template < class Event, class FSM >
      ...
      struct do_load_url
      {
        ...
        void operator() (EVT const& evt, FSM& fsm, SourceState&, TargetState&)
        {
          ...
          {
            (*(fsm.pp_ops_))
                ->do_load_url (evt.url_, evt.mime_type_, evt.title_,
                               evt.album_art_);
          }
        }
      };
      ...
      struct transition_table
        : boost::mpl::vector<
              ...
              bmf::Row< running, err_evt, bmf::none, bmf::none,
              ...
              //    +----+---------------------+------------------+-------------+------------------------+--------------------+
              bmf::Row< polling, err_evt, quitted, do_report_fatal_error,
                        is_fatal_error >,
    ...
    // boost::msm::back::mpl_cast_fsm_check> fsm;
